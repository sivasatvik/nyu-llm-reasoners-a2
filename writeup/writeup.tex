        ...
        \item [1.2.1] \textbf{pytorch\_compile}
            \begin{enumerate}[label=\alph*.]
                \item The table for uncompiled vs compiled version of attention is given below
                    ...
                    \end{table}
                \newpage
                \item The table for uncompiled vs compiled version of the model is given below. The forward pass times for the compiled version are somewhat lower than the uncompiled version across all model sizes, which is expected since PyTorch's compilation can optimize the computation graph and fuse operations to improve performance. The forward+backward pass times for the compiled version are also somewhat lower than the uncompiled version across all model sizes, which is also expected since the optimizations from compilation can benefit both forward and backward computations. However, the performance improvement from compilation may not be very significant for smaller models, but it becomes more pronounced as the model size increases due to the larger computational load and greater opportunities for optimization.
                    \begin{table}[h]
                        \centering
